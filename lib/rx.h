...
struct qrx_stream {
	...
	int64_t id;			// id of the stream
	uint64_t end;		// full stream size - unknown until we see a fin

	uint64_t consumed;	// offset into the stream we've consumed
	uint64_t have;		// offset into the stream that we have all data for
...
static inline bool qrx_is_finished(qrx_stream_t *r) {
	return r->offset == r->finish;
}

void *qrx_buffer(qrx_stream_t *r, size_t min, size_t *psz);
...
int qrx_received(qrx_stream_t *r, bool fin, uint64_t offset, void *p, size_t sz);
